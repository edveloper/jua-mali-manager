-- Fix ambiguous "id" references inside atomic sale functions by fully qualifying columns.
...
BEGIN
  ...
  WHERE s.id = p_service_id AND s.shop_id = p_shop_id
  FOR UPDATE;
  ...
  SET capacity = COALESCE(s.capacity, 0) - p_quantity
