import { LayoutDashboard, Package, AlertTriangle, Receipt } from 'lucide-react';
import { cn } from '@/lib/utils';

export type TabType = 'dashboard' | 'products' | 'alerts' | 'sales';
...
export function Navigation({ activeTab, onTabChange, alertCount }: NavigationProps) {
  const tabs = [
    ...
    { id: 'sales' as TabType, label: 'Sales', icon: Receipt },
  ];
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={cn(
                "flex flex-col items-center gap-1 py-2 px-4 rounded-xl transition-all duration-200 relative",
                isActive 
